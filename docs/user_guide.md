...
2. 我们会对每条触发消息进行机器审核或者人工审核，确保机器人的使用不会导致社群环境的恶化。对于恶意使用机器人的用户，我们会警告3次后进行拉黑处理。
3. 过于频繁地请求机器人也会触发警告并进行拉黑处理。目前的逻辑是超限后继续请求达到10次进行拉黑。
...
1. 纯数字ID可能会被AI审核识别为广告而不予通过，这类ID的数据无法查询
