...
@AxbotInteractiveFunc(command = InteractiveCommand.WT_UPDATE_PROFILE)
@Component
@Slf4j
public class FuncWtUpdateProfile extends AbstractInteractiveFunction {
                ...
                try {
                    ...
                    while (i < maxTimes) {
                        ...
                        if (Mission.STATUS_SUCCESS.equals(status)) {
                            ...
                            if (opt.isPresent()) {
                                content = commonWtProfile.kookProfileFound(nickname, opt.get());
                                break;
                            }
                        }
                        try {
    ...
    private String kookProfileInQuery(String nickname) {
        ...
        card.addModuleContentSection("本条消息为临时消息，仅你可见");
